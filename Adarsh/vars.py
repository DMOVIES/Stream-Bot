...
class Var(object):
    ...
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002028023012'))
    PORT = int(getenv('PORT', 8080))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", ""))  # 20 minutes
    OWNER_ID = set(int(x) for x in os.environ.get("OWNER_ID", "5412093415").split())  
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = None
    OWNER_USERNAME = str(getenv('OWNER_USERNAME', 'd_movie_office'))
    if 'DYNO' in environ:
        ON_HEROKU = True
    ...
    DATABASE_URL = str(getenv('DATABASE_URL', 'mongodb+srv://stream:stream@cluster0.pd23blr.mongodb.net/?retryWrites=true&w=majority'))
